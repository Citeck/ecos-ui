...
export default class RecordImpl {
  ...
  getAttributesToSave(): Record<string, AttributeLike> {
    ...
    if (attributesToSave && workspace.getEnabledWorkspaces() && !attributesToSave['_workspace'] && !attributesToSave['_workspace?str']) {
      attributesToSave['_workspace'] = workspace.getWorkspaceId() as any;
    }
  ...
  async _getChildAssocAttributes(): Promise<string[]> {
    ...
    const modelAtts = await this._records
      .get(RESOLVED_TYPE_SOURCE_ID + '@' + typeId)
