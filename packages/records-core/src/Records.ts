...
import { AttributesType, PreProcessAttsType, PreProcessAttsTypeNotNil, RecordsQueryResponse } from './types';
...
class RecordsComponent {
  ...
  async query<T = any>(
    ...
    const response = await recordsQueryFetch(queryBody, options?.signal as any);
...
const Records: RecordsContainerType = new RecordsComponent();

recordsClientManager.init(Records);

/**
 * Web-only opt-in: expose the singleton on `window.Citeck.Records` / `window.Records`
 * for legacy code and devtools. Never called from the core; the host app invokes it
 * at startup. No-op outside a browser, so it is safe to import in react-native.
 */
