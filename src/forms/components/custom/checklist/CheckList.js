...
export default class CheckListComponent extends BaseComponent {
  ...
  static get builderInfo() {
    return {
      ...
      weight: 100,
      schema: CheckListComponent.schema()
    };
  }
  ...
  get defaultSchema() {
    return CheckListComponent.schema();
  }
  ...
  refreshDOM() {
    ...
    if (this.listContainer && this.uploadContainer) {
      // Refresh file list.
      const newList = this.buildFileList();
    ...
    return this.ce(
      'li',
      {
        ...
        onDragover(event) {
          ...
          this.className = 'list-group-item fileDragOver';
        ...
        onDrop(event) {
          console.log('onDrop event', event, this);
          this.className = 'list-group-item';
          event.preventDefault();
  ...
  createImageListItem(fileInfo, index) {
    ...
    const fileService = this.fileService;
    if (fileService) {
      fileService.downloadFile(fileInfo).then(result => {
          ...
          ? this.ce(
              ...
              [
                this.ce(
                  ...
                  [this.ce('i', { class: this.iconClass('book') }), this.text('Gallery')]
                ),
                this.ce(
                  'button',
                  {
                  ...
                  [this.ce('i', { class: this.iconClass('camera') }), this.text('Camera')]
                )
              ]
            )
          : this.ce('div')
  ...
  addWarnings(container) {
    ...
    if (!this.component.storage) {
      ...
      warnings.appendChild(
        this.ce('p').appendChild(this.text('No storage has been set for this field. File uploads are disabled until storage is set up.'))
      );
    }
    if (!this.support.dnd) {
      hasWarnings = true;
