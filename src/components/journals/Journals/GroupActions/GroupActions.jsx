...
const GroupActions = React.memo(
  props => {
    ...
    const {
      ...
      excludedRecords,
      journalName,
      menuClassName: extraMenuClassName
    } = props;
    ...
    const iconOpener = useCallback(flag => (flag ? 'icon-small-up' : 'icon-small-down'), []);
    ...
    return (
      <>
        <DropdownOuter
          ...
          className={classNames('ecos-group-actions__dropdown ecos-group-actions__dropdown_new', {
            'ecos-group-actions__opened-focus': isOpenRecActions && !isEmpty(recordsActions)
          })}
          menuClassName={classNames('ecos-group-actions__dropdown-menu', extraMenuClassName)}
        ...
        >
          <Tooltip uncontrolled showAsNeeded target={targetPrefix + '-rec'} text={label} contentComponent={labelRecActionsCount}>
            <IcoBtn
              invert
              className={classNames('ecos-group-actions__control_new ecos-btn_grey3 ecos-group-actions__control')}
        ...
        {isSeparateActionListForQuery && !isEmpty(queryActions) && (
          ...
          >
            <TwoIcoBtn
              className={classNames('ecos-btn_hover_blue2 ecos-btn_grey3 ecos-group-actions__control ecos-group-actions__control_new', {
