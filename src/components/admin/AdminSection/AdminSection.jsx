...
import { Labels, SectionTypes } from '@citeck/constants/adminSection';
...
class AdminSection extends React.PureComponent {
  ...
  render() {
    const { activeSection, tabId, isActivePage, isOpenMenu, isAccessible, isAccessibleSectionType, stateId } = this.props;
    const { journalStateId, additionalHeights, needResetJournalView } = this.state;

    const enabledWorkspaces = getEnabledWorkspaces();
    const isViewHeadDevTools = get(activeSection, 'type') === SectionTypes.DEV_TOOLS;

    return (
      <div className={classNames('ecos-admin-section__container ecos-admin-section__container_new')} ref={this.setWrapperRef}>
        <div
        ...
        >
          <Container fluid={this.isFluid()} className="p-0">
            {!enabledWorkspaces && (isAccessible || isAccessibleSectionType) && isViewHeadDevTools && (
        ...
        {!enabledWorkspaces && isAccessible && <AdminMenu />}
      </div>
    );
  }
}
