...
const { is: is$b, isAny: isAny$4 } = require$$0;
...
var endEventRequired = function () {
  ...
  function check(node, reporter) {
    if (!isAny$4(node, ['bpmn:Process', 'bpmn:SubProcess'])) {
      return;
    }
...
var superfluousGateway = function () {
  function check(node, reporter) {
    ...
    if (incoming.length === 1 && outgoing.length === 1) {
      reporter.report(node.id, t('bpmn-linter.gateway.superfluous'));
    }
  }
