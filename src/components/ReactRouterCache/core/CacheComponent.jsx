...
export const isMatch = match => isExist(match) && get(match, COMPUTED_UNMATCH_KEY) !== true;
...
export default class CacheComponent extends Component {
  ...
  constructor(props, ...args) {
    ...
    if (typeof document !== 'undefined') {
      this.__placeholderNode = document.createComment(
        ` Route cached ${props.cacheKey ? `with cacheKey: "${get(props.cacheKey, 'cacheKey', props.cacheKey)}" ` : ''}`
      );
    }
  ...
  render() {
    ...
    const { className: behaviorClassName = '', ...behaviorProps } = value(run(behavior, undefined, !matched), {});
