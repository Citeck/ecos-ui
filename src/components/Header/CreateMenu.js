...
import { DropdownMenu as EcosDropdownMenu } from '../common';
    ...
    return (
      <Dropdown className="ecos-header-create ecos-header-dropdown" isOpen={dropdownOpen} toggle={this.toggle}>
        <DropdownToggle tag="div">
          <IcoBtn icon={'icon-small-plus'} className={classNameIcoBtn} disabled={disabled}>
            {!isMobile && t('create_case.label')}
          </IcoBtn>
        </DropdownToggle>
        <DropdownMenu className={classNameMenu}>
          <EcosDropdownMenu items={items} mode={mode} setCascade={{ collapseOneItem: true }} />
        </DropdownMenu>
      </Dropdown>
    );
  }
