...
describe('TasksDashlet record event listeners', () => {
  const record = 'emodel/doc@tasks-listeners';
  const store = configureStore()({ view: { isMobile: false }, app: { dashboardEditable: false } });
  const mount = () =>
    render(
      <Provider store={store}>
        <TasksDashlet id="widget-1" record={record} tabId="tab-1" dashboardId="dash-1" />
      </Provider>
    );
