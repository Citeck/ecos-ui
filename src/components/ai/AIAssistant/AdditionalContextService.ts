...
class AdditionalContextService {
   ...
   */
  addContextType(contextType: string, selectedTypes: string[], setSelectedTypes: Dispatch<string[]>): void {
  ...
  async handleAddAttributeContext(
    ...
    this.addContextType(ADDITIONAL_CONTEXT_TYPES.ATTRIBUTES, selectedTypes, setSelectedTypes);
  ...
  toggleRecordContext(
    ...
    const existingRecordIndex = additionalContext.records.findIndex(record => record.recordRef === recordData.recordRef);
  ...
  toggleDocumentContext(
    ...
    const existingDocumentIndex = additionalContext.documents.findIndex(doc => doc.recordRef === documentData.recordRef);

    if (existingDocumentIndex !== -1) {
      setAdditionalContext((prev: AdditionalContext) => {
        const updatedDocuments = prev.documents.filter(d => d.recordRef !== documentData.recordRef);
        if (updatedDocuments.length === 0) {
          setSelectedTypes((prevTypes: string[]) => prevTypes.filter(c => c !== ADDITIONAL_CONTEXT_TYPES.DOCUMENTS));
        }
      ...
      this.addContextType(ADDITIONAL_CONTEXT_TYPES.DOCUMENTS, selectedTypes, setSelectedTypes);
    }
  }
}
