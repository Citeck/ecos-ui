  ...
  if (gridRows.length > 0) {
    const inlineTools = () => {
      ...
      if (shouldShowViewButton) {
        iconButtons.push(<IcoBtn key={'view'} icon={'icon-on'} className={'${inlineTlsActionClassName} inline-tools-actions-btn__on'} onClick={onClickView} />);
      }

      const shouldShowEditButton = isBoolean(get(displayElements, 'edit')) ? displayElements.edit : !viewOnly;
      if (shouldShowEditButton) {
        iconButtons.push(<IcoBtn key={'edit'} icon={'icon-edit'} className={'${inlineTlsActionClassName} inline-tools-actions-btn__edit'} onClick={onClickEdit} />);
      }

      const shouldShowDeleteButton = isBoolean(get(displayElements, 'delete')) ? displayElements.delete : !disabled && !viewOnly;
      if (shouldShowDeleteButton) {
        iconButtons.push(<IcoBtn key={'delete'} icon={'icon-delete'} className={'${inlineTlsActionClassName} inline-tools-actions-btn__delete'} onClick={onClickDelete} />);
      }
