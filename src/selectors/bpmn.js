import { SEARCH_MIN_LENGTH } from '@citeck/constants/bpmn';
...
const normalizeSearchText = createSelector([selectSearchText], searchText => {
  const trimmed = searchText ? searchText.toLowerCase().trim() : '';
  return trimmed.length >= SEARCH_MIN_LENGTH ? trimmed : '';
});

export const selectModelsBySearchText = createSelector([selectAllModels, normalizeSearchText], (allModels, normalizedSearchText) => {
...
export const selectModelsInfoByCategoryId = createSelector(
  ...
  (modelsMap, categoryId, normalizedSearchText) => {
    ...
    const filteredModels = modelsInfo.models.filter(
      model =>
