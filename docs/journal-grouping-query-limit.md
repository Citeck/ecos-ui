...
Проверено в браузере на локальном стенде (`ept-active-sprint-journal`, TEST2, группировка по
`priority`): запрос уходит с `"page": {"skipCount": 0, "maxItems": 100, "page": 1}`, грид рисует
группы.
