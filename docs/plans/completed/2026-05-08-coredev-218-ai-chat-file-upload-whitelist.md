...
- [x] write tests for `getFileExtension(filename)` — пустая строка, нет расширения, несколько точек, верхний регистр (`'IMG.JPG'` → `'.jpg'`)
- [x] write tests for `buildAcceptString(whitelistGroups)` — корректная склейка из всех групп, deduplication
- [x] write tests for `isExtensionAllowed(filename, whitelistGroups)` + `isExtensionBlocked(filename, blocklist)` — happy + edge cases (нет ext, регистр)
- [x] add `FILE_UPLOAD_WHITELIST` (documents/images/tables/presentations/text_code/existing) — см. Technical Details
- [x] add `FILE_UPLOAD_BLOCKLIST` (executables/archives/media/svg) и `FILE_UPLOAD_LIMITS` (10/5/50/200)
- [x] implement helpers: `getFileExtension`, `buildAcceptString`, `isExtensionAllowed`, `isExtensionBlocked`
- [x] run `yarn test src/components/AIAssistant/__tests__/fileUploadConfig.test.js` — must pass before Task 2

### Task 2: Расширить validateFile в useFileUpload (per-file validation)
- [x] write tests for `validateFile`:
...
- [x] update `validateFile` в `useFileUpload.js`:
...
- [x] run `yarn test useFileUpload` — must pass before Task 3

### Task 3: Добавить validateBatch (cross-file validation) и интеграция
- [x] write tests for `validateBatch(files, alreadyUploaded)`:
...
- [x] write tests for `handleFileUpload` integration: при invalid batch `uploadFileToRecords` не вызывается
- [x] implement `validateBatch` в `useFileUpload.js`
- [x] вызвать `validateBatch(files, uploadedFiles)` в начале `handleFileUpload` до per-file валидации
- [x] run `yarn test useFileUpload` — must pass before Task 4

### Task 4: Динамический accept в ChatInput
- [x] write/update test for `ChatInput`: `accept` prop собирается через `buildAcceptString`, содержит `.png, .jpg, .xlsx, .csv`, не содержит `.exe, .svg, .zip`
- [x] update `ChatInput.jsx:81`:
...
- [x] run `yarn test ChatInput` — must pass before Task 5

### Task 5: Drop-zone подсветка только для валидных типов
- [x] write tests for `hasValidDraggedFile(dataTransferItems)`:
  ...
  - пустой items → false
- [x] update `handleDragOver` в `useFileUpload.js`:
...
- [x] inline-комментарий о MIME-only ограничении `dataTransfer.items` в `dragover`
- [x] run `yarn test useFileUpload` — must pass before Task 6

### Task 6: Verify acceptance criteria
- [x] verify accept-list расширен на images + tables + presentations + text/code (smoke в тестах ChatInput)
- [x] verify svg/exe/zip/медиа отвергаются (smoke в тестах validateFile)
- [x] verify файл > 10 MB → toast до отправки на сервер (моки `uploadFileToRecords` не вызывается)
- [x] verify 5+ файлов одновременно → toast, ничего не загружается
- [x] verify сообщения об ошибках содержат имя файла, размер, лимит
- [x] verify PDF/DOCX upload продолжает работать (regression в существующих тестах)
- [x] run `yarn test src/components/AIAssistant` — все тесты зелёные (543/543 passed across 30 suites)
- [x] run `yarn lint` на затронутые файлы — исправить все warnings (использован eslint напрямую — нет yarn-script `lint`; auto-fix применён, 0 warnings/errors)

### Task 7: [Final] Documentation pass
- [x] обновить inline JSDoc в `useFileUpload.js` (новые опции, поведение)
- [x] обновить JSDoc в `constants.js` для новых экспортов
